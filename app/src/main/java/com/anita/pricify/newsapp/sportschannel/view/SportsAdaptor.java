...
public class SportsAdaptor extends RecyclerView.Adapter<SportsAdaptor.SportsViewHolder> {

    private List<NewsModel> sportsModelList;
    ...
    public SportsAdaptor(Context context) {
        this.context = context;
        this.sportsModelList = new ArrayList<>();
        getSportsListData(this);
    }

    @Override
    public SportsAdaptor.SportsViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View itemView = LayoutInflater.from(parent.getContext())
                .inflate(R.layout.activity_news_item, parent, false);

        return new SportsAdaptor.SportsViewHolder(itemView);
    }

    @Override
    public void onBindViewHolder(final SportsAdaptor.SportsViewHolder holder, final int position) {
        sportsModel = sportsModelList.get(position);
        holder.mTextAuthor.setText(sportsModel.getAuthor());
        holder.mTextTitle.setText(sportsModel.getTitle());
        String strDate = sportsModel.getPublishedAt();
        ...
        holder.setNewsModel(sportsModel);
        holder.setIsRecyclable(false);
        mCardView.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Bundle bundle = new Bundle();
                bundle.putSerializable("sports",  sportsModelList.get(position));
                Intent intent = new Intent(context, SportsDetailsActivity.class);
    ...
    public void getSportsListData(final SportsAdaptor newsAdaptor) {
        pDialog = new ProgressDialog(context);
        pDialog.setMessage("Loading...");
        pDialog.show();

        StringRequest stringRequest = new StringRequest(Request.Method.GET, WebUrl.WEB_SPORTS,
                    ...
                    public void onResponse(String response) {
                        ...
                        try {
                            ...
                            for (int i = 0; i < newsArray.length(); i++) {
                                ...
                                sportsModel = new NewsModel(newsObject.getString("author"), newsObject.getString("title"),
